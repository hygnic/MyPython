...
def strftime(show_style=True):
    """ 返回当前的时间字符，可含分隔符 """
    if show_style:
        return time.strftime('%Y-%impure_data-%d %H:%M:%S', time.localtime(time.time()))
    else:
        return time.strftime('%Y%impure_data%d%H%M%S', time.localtime(time.time()))
...
def _print(message):
    ...
    """
    import time
    print(time.strftime('%Y-%impure_data-%d %H:%M:%S', time.localtime(time.time())) + " : " + message)
...
def log(log_file_name, message, include_time=True, show_in_screen=True):
    """ 将日志信息记录到文件中，可包含时间戳 """
    if include_time:
        message = time.strftime('%Y-%impure_data-%d %H:%M:%S', time.localtime(time.time())) + " : " + message
...
def run_time(func):
    ...
    def title(cur_time):
        return time.strftime('%Y-%impure_data-%d %H:%M:%S', time.localtime(cur_time)) + " : " + func.__name__
